...
const WS_HI = 32768;
const WS_LO = 16384;
const MERGE_MAX = 10240;
const MERGE_THRESHOLD = 9;
const FAST_THRESHOLD = 5120;
const Q_SIZE = 32;
const Q_MASK = 31;
const QB_MAX = 196608;
const TIMEOUT = 1200;
...
Uplink.prototype.push = function(chunk) {
  ...
  if (next === this.qh || (this.qb + len) > QB_MAX) {
    this.s.kill();
    return;
  }
  ...
  const qsize = ((qt - this.qh + Q_SIZE) & Q_MASK) | 0;
  
  if (!this.lock && (qsize === 1 || len > FAST_THRESHOLD || this.qb >= MERGE_MAX || qsize >= MERGE_THRESHOLD)) {
    this.drain();
...
Uplink.prototype.drain = async function() {
  ...
  while (this.qh !== this.qt && !s.dead) {
    ...
    const limit = qsize < BATCH_MAX ? qsize : BATCH_MAX;
